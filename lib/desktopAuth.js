// Helpers for Google/GitHub/Microsoft sign-in when running inside the
...
//
// The fix: never load Google/GitHub/Microsoft's own pages inside the
// Electron window at all. Sign-in opens in the user's real, already-trusted
// browser via `blueprintDesktop.openExternal` (see electron/preload.js),
// completes a normal server-side OAuth code exchange
// (app/api/auth/google/desktop-*, app/api/auth/github/desktop-*,
// app/api/auth/microsoft/desktop-*), and hands control back to the app via
// a custom `blueprint://` link that the OS routes straight back into
...
//
// Email/password sign-in doesn't need any of this — it's a plain form
// submitted directly to Firebase, no popup and no third-party login page
// involved, so it works identically inside and outside the desktop shell.
...
/**
 * Sets the desktop window's title bar to "Blueprint — {name}", or back to
 * plain "Blueprint" if called with no name. No-ops outside the desktop
 * shell. Note: this is the window's own title, not the bold app name macOS
 * shows in the global menu bar next to the Apple logo — that one's fixed
 * to "Blueprint" at the OS level and can't be changed per-project. See
 * electron/main.js's "set-window-title" handler.
 */
export function setDesktopTitle(name) {
  if (!isDesktopApp()) return;
  window.blueprintDesktop.setTitle(name ? `Blueprint — ${name}` : "Blueprint");
}
