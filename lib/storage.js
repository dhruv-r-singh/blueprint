...
export const MAX_UPLOAD_BYTES = 15 * 1024 * 1024; // 15MB — generous for images/docs, keeps Storage costs sane
...
export function uploadFile(path, file, onProgress) {
  return new Promise((resolve, reject) => {
    if (file.size > MAX_UPLOAD_BYTES) {
      reject(new Error(`That file is too big — max ${Math.round(MAX_UPLOAD_BYTES / 1024 / 1024)}MB.`));
      return;
    }
