...
import { GITHUB_EXTRA_SCOPES, GOOGLE_EXTRA_SCOPES } from "./oauthScopes";
...
// See lib/oauthScopes.js — GOOGLE_EXTRA_SCOPES is the single source of
// truth shared with the desktop OAuth flow, so this and
// app/api/auth/google/desktop-*.js can't drift out of sync again.
GOOGLE_EXTRA_SCOPES.forEach((scope) => googleProvider.addScope(scope));

export const githubProvider = new GithubAuthProvider();
// See lib/oauthScopes.js — GITHUB_EXTRA_SCOPES is the single source of
// truth shared with the desktop OAuth flow (app/api/auth/github/desktop-*.js).
// `delete_repo` specifically is needed for the "Connect GitHub" toggle in
// project Settings to be able to delete the repo again when someone
// switches it off — GitHub gates repo deletion behind this scope
// separately from plain "repo", on purpose (deleting a repo is
// irreversible). Anyone who connected GitHub before this scope existed
// needs to reconnect once (Preferences → Connected accounts) before
// deletion works for them — see hasGithubScope() in lib/integrations.js,
// which detects this proactively instead of only finding out when a
// delete fails.
GITHUB_EXTRA_SCOPES.forEach((scope) => githubProvider.addScope(scope));
